...
전화를 건 고객의 번호가 보이게 하려면 `callerIdMode: 'original'` 을 씁니다.

```typescript
await call.transfer('021234567', { callerIdMode: 'original' });
```

바뀌는 것은 **받는 쪽에 표시되는 번호**뿐입니다. 통화 자체는 계정 번호에서 넘기는 전환이라,
받는 장비에 따라 계정 번호가 보일 수 있습니다.
...
걸려온 통화의 발신자 번호는 다음 경우에 표시할 수 없습니다. 이때 `callerIdMode: 'original'` 은 조용히
...
- 아웃바운드 통화 (걸려온 통화가 없어 무시됩니다)

`callerId` 로 직접 지정할 수 있는 번호는 **계정이 보유한 번호**이거나 **그 통화의 발신자**
뿐입니다. 그 외 번호는 발신번호 변작 방지를 위해 거절됩니다. 둘 다 지정하면 `callerId` 가
이기고 `callerIdMode` 는 무시됩니다.

> VoiceML `<Dial>` 은 기본값이 다릅니다 — 통신사 직결 인바운드면 **걸려온 통화의 발신자 번호를 기본으로 표시**합니다.
