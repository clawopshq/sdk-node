...
export class CallSession {
   ...
   *
   * `callerIdMode: 'original'` 은 인바운드 통화의 **발신자 번호를 표시하려는 선호**다.
   * 표시할 수 없는 통화(KCT 직결 인입이 아니거나 국내 번호로 정규화되지 않는 발신번호)면
   * 조용히 계정 보유번호로 내려앉고 **전환은 그대로 성사된다**. 바뀌는 것은 받는 쪽 표시뿐이라
   * (통화 자체는 계정 번호에서 넘기는 전환이다) 받는 장비에 따라 계정 번호가 보일 수 있다.
   *
   * `callerId` 는 번호를 직접 주는 **지시**라 성격이 다르다. 허용 범위(계정 보유번호 또는
   * KCT 직결 인입 통화의 발신자)를 벗어나면 전환 자체가 실패한다. 둘 다 주면 `callerId` 가
